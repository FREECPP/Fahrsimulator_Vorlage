...
ELL_CENTER = (373.9062805175781, 411.810302734375)
ELL_AXES   = (32.260927200317383, 142.35995483398438)
ELL_ANGLE  = 70.36243438720703
...
def extract_tuple(df): 
    ...
    cols = ["nose", "left_eye", "right_eye", "left_ear", "right_ear", "left_shoulder", "right_shoulder", "left_wrist", "right_wrist", "left_thumb", "right_thumb", "timestamp", "label"]
    df = df.select(cols)
    for col in cols:
        if col != "label" and col != "timestamp":  # "label" und "timestamp" bleiben unverändert
            # Entferne Klammern und trenne die Werte
            df = df.with_columns([
                pl.col(col)
            ...
            df = df.with_columns([
                pl.col(col)
                .list.eval(               # wende Expression auf JEDES Element der Liste an
                    pl.element()
                        .str.strip_chars()  # hier: Leerzeichen weg
                )
                .alias(col)
            
            ])
            # Konvertiere die Werte in x, y, d und entferne Leerzeichen
            df = df.with_columns([
...
def point_to_ellipse_distance_newton(x: float, y: float, a: float, b: float, iters: int = 15) -> float:
    ...
    # Startwert (guter Standard-Guess)
    ...
    for _ in range(iters):
        ...
        # f(t) = (ex - x)*(-a sin t) + (ey - y)*(b cos t) = 0 (Orthogonalitätsbedingung)
...
def add_ellipse_distance_exact(
    ...
    # Halbachsen (bei dir schon korrekt!)
    a = float(ELL_AXES[0])
    b = float(ELL_AXES[1])
...
if __name__ == "__main__":
    df = pl.read_csv("driverDistractionModel/model_train/train_data/train_data2.csv")
    df = pre_process(df)
    cols = [
        "head_pitch_deg",
        "head_distance",
    ...
    df.write_csv("driverDistractionModel/model_train/train_data/extracted_features2.csv")
