...
// Slice the rolling telemetry buffer to the requested window (binary search on
// the sorted elapsedMs) and downsample for rendering.
...
const SilabSignalChart = memo(function SilabSignalChart({ signal }) {
  ...
  useEffect(() => {
    const refresh = (force = false) => {
      ...
      const { data: windowed, latest } = windowedData(telemetryStore.getData(), windowMs)
      setData(windowed)
      setLatestTime(latest)
    }

    refresh(true)
    const unsubscribe = telemetryStore.subscribe(() => refresh(false))
    return unsubscribe
  }, [windowMs])
      ...
      <div className="silab-line-chart-section">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
            ...
            />
            {signal.domain && signal.domain[0] < 0 && (
...
export default SilabSignalChart
